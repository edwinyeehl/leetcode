...
var minCost = function(houses, cost, m, n, target) {
    ...
    function dfs(idx = 0, prevColor = -1, neighborhoods = 0) {
        ...
        if (color > 0) // If the house is already painted
        ...
        for (let i = 1; i <= n; i++) {
            ...
            if (i !== prevColor)
                potentialCost = dfs(idx + 1, i, neighborhoods + 1);
            // else, the neighborhood simply expanded so the neighborhood count stays the same
            else
                potentialCost = dfs(idx + 1, i, neighborhoods);
            
            if (potentialCost === Infinity)
                continue;
...
console.log(minCost([0,0,0,0,0],[[1,10],[10,1],[10,1],[1,10],[5,1]],5,2,3));